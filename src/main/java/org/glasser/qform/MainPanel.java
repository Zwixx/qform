...
public class MainPanel extends MDIPanel implements ActionListener, InternalFrameListener, QueryPanelListener {
	...
	private static Object[][] exportSelectedConfig_ = { { "EXPORT_SELECTED_INSERT" }, { "EXPORT_SELECTED_CSV" } };
	...
	private static Object[][] exportAllConfig_ = { { "EXPORT_ALL_INSERT" }, { "EXPORT_ALL_CSV" } };

	private static Object[][] selectedOrAllConfig = { { "_Selected Row(s)", exportSelectedConfig, "S", null },
			{ "_All Rows", exportAllConfig, "A", null } };

	private static Object[][] selectedOrAllConfig_ = { { "SELECTED_ROWS", exportSelectedConfig_ },
			{ "ALL_ROWS", exportAllConfig_ } };

	private static Object[][] gridViewSubConfig = {
			{ "Select visible columns...", "COLUMN_MAP_DIALOG", "V",
					"Select which columns will be displayed in this table." },
			{ "CHECKBOX_Horizontal scrollbar", "TOGGLE_HORIZONTAL_SCROLLBAR", "H",
					"Show or hide the horizontal scrollbar." } };

	private static Object[][] gridViewSubConfig_ = { { "COLUMN_MAP_DIALOG" },
	...
	static Object[][] queryformMenuConfig = { { "New Query Form...", "NEW_QUERY_FORM", "Q", "Open a new query form." },
	...
	static Object[][] queryformMenuConfig_ = { { "NEW_QUERY_FORM", }, { "WINDOW_TITLE", }, { "OPEN_FOR_QUERY", },
			{ "WHERE_CLAUSE_QUERY", }, { "ADD_RECORD", }, { "MODIFY_RECORD", }, { "DELETE_RECORD", },
			{ "CLONE_RECORD", }, { "GRID_VIEW", gridViewSubConfig_ }, { "EXPORT", selectedOrAllConfig_ },
	...
	private static Object convert(Object[][] config, String menuName) {
		...
		for (int j = 0; j < config.length; j++) {
			...
			if (subConfig != null)
				row[1] = convert((Object[][]) subConfig, action + "_SUB");
			else
				row[1] = action;
	...
	private static void convertMenu(Object[][] config, String menuName) {
		for (int j = 0; j < config.length; j++) {
			...
			System.out.println("\t\t,{\"menu." + menuName + "." + command + ".label\",\t\t\t\"" + label + "\"}");
			System.out.println("\t\t,{\"menu." + menuName + "." + command + ".mnemonic\",\t\t\t\"" + mnemonic + "\"}");
			System.out.println("\t\t,{\"menu." + menuName + "." + command + ".tooltip\",\t\t\t\"" + tooltip + "\"}");
		}
	}
	...
	static String[][] lafSubConfig_ = { { "INSTALL_LOOK_AND_FEEL" }, { "SEPARATOR" } };

	static Object[][] windowMenuConfig = { { "Minimize all", "MINIMIZE_ALL", "N", "Minimize all windows" },
			{ "Maximize all", "MAXIMIZE_ALL", "X", "Maximize all windows" },
			{ "Restore all", "RESTORE_ALL", "R", "Restore all windows" },
			{ "Cascade", "CASCADE", "C", "Cascade all windows" }, { "SEPARATOR" },
	...
	static String[][] helpMenuConfig = { { "System Information...", "SYSINFO", "I", "Display system information." },
			{ "About...", "ABOUT", "A", "Display program information." } };

	static String[][] helpMenuConfig_ = { { "SYSINFO" }, { "ABOUT" } };

	private boolean[][] helpMenuStates = { { true, true } // NO_CONNECTIONS = 0;
	...
	private final static String[] STATE_MESSAGES = {
			...
			"Enter data for new record. Fields with red borders are required. ", "Modify the current record. ",
			"Enter query criteria. \"%\" may be used as a wildcard in character fields, \"<\" and \">\" in numeric fields. ",
			"Modify the primary key and other unique fields before saving cloned record. ", null };
	...
	public MainPanel(Frame parent, File configFile) {
		...
		if (configFile.exists()) {
			try {
				...
				er.printStackTrace();
				String msg = "A Factory Configuration Error occurred while reading your configuration file: "
						+ configFile
		...
		String msg = "<center><h1>QueryForm</h1><h3>Version " + VersionNumber.VERSION_NUMBER
				+ "</h3><h3>Copyright � 1998 - 2004, David F. Glasser (dglasser@pobox.com)"
		...
		GUIHelper.buildButtonPanel(bPanel, new ButtonConfig[] { new ButtonConfig(okButton, 'O', null, null) }, closer);
		...
		// now that all of the dialogs have been created, store them in an array
		// for
		// easy changing of the look and feel.
		Component[] temp = { configDialog, columnMapDialog, tableSelector, loginDialog, whereClauseDialog, exportDialog,
				overwriteDialog, aboutDialog, tpLafDialog, sysInfoDialog };
		...
		String horizontalScrollbarLabel = bundle.getString("menu.GRID_VIEW_SUB.TOGGLE_HORIZONTAL_SCROLLBAR.label");

		horizontalScrollbarMenuItem = (JCheckBoxMenuItem) GUIHelper.findMenuItemByLabelText(queryformMenu,
		...
		for (int j = 0; j < lafs.length; j++) {
			...
			if (Util.isNothing(lafName))
				lafName = "Unknown";
			String className = info.getClassName();
			if (Util.isNothing(className)) {
				System.err.println(
						"Error in configuration file: A third-party-laf element is missing its laf-class attribute.");
				continue;
			}
	...
	private ActionListener lafManager = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (debug)
				System.out.println("Changing look-and-feel: " + lafClassName);
			try {
				...
				try {
					...
					if (laf.isSupportedLookAndFeel()) {
						...
						// if this is a third-party LAF, this will update it in
						// the config file.
						// This may be necessary if it was added to the config
						// file manually.
						...
						} else { // disable the menu item, but leave it in the
									// menu.
	...
	private void restoreDefaultLaf() {
		try {
			LookAndFeel defaultLAF = (LookAndFeel) Class.forName(UIManager.getCrossPlatformLookAndFeelClassName())
					.newInstance();
			setLookAndFeel(defaultLAF);
		} catch (Throwable ex) {
			ex.printStackTrace();
			GUIHelper.errMsg(parent,
					"The attempt to restore the default Look-and-Feel was unsuccessful. QueryForm will now terminate.",
					"Unrecoverable Error");
			System.exit(9);
		}
	}
	...
	private void selectCurrentLafMenuItem() {
		...
		if (currentLAF == null) {
			System.err.println(
					"WARNING: Could not select menu item for current look-and-feel. UIManager.getLookAndFeel() returned null.");
			return;
		}

		JRadioButtonMenuItem menuItem = (JRadioButtonMenuItem) lafMenuItems.get(currentLAF.getClass().getName());
		if (menuItem != null) {
	...
	public void showUnsupportedLafErrorMessage(String lafName, String lafClassName) {
		if (lafName == null)
			lafName = "The selected look-and-feel";

		GUIHelper.errMsg(parent, lafName + " (" + lafClassName + ") is not supported on this platform.",
	...
	public void setScreenState() {
		...
		if (jif != null) {
			...
			if (allFrames != null && allFrames.length > 0) {
				setScreenState(MainPanel.ALL_FRAMES_ICONIFIED);
			} else {
				if (dsMap.size() > 0) {
					this.setScreenState(MainPanel.HAS_CONNECTIONS);
				} else {
					this.setScreenState(NO_CONNECTIONS);
				}
			}
...

	{
		try {
			...
			if (filepath != null) {
				...
				// otherwise check for a file called
				// qform_editable_types.properties,
				// first in the user's home directory, then in the current
				// working directory.
			...
			if (file.exists() && file.canRead()) {
				...
				// if the System property was used, warn the user that the file
				// wasn't found.
				if (filepath != null) {
	...
	public void _actionPerformed(ActionEvent e) {
		...
		if (command.equals("DATA_SOURCE_DIALOG")) {
			...
			if (ld != null) {
				if (establishedConnections.contains(ld)) {
					int reply = JOptionPane.showConfirmDialog(this,
							"A connection to this data source has already been \nestablished. Do you want to create another one?",
				...
				try {
					...
					if (ld.isLoginRequired()) {
						...
						if (userName != null && password != null) {
							// if we already have a username and password,
							// attempt to login.
					...
					GUIHelper.errMsg(parent,
							"The JDBC driver class (" + ld.getDriverClassName() + ") was not found in the classpath.",
							"Login Failed");
			...
			for (int j = 0; j < frames.length; j++) {
				...
				try {
					...
					// else {
					// System.out.println("frame " + j + " is already
					// restored.");
					// }
			...
			try {
				...
				GUIHelper.errMsg(this,
						"This look-and-feel class was not found in QueryForm's classpath:\n\n" + className, null);
			...
			qp.runQuery(whereClause, QueryPanel.NO_RESULTSET);
		...
		if (command.equals("COLUMN_MAP_DIALOG")) {
			...
			// System.out.println("VisibleGridColumns is " +
			// visibleGridColumns);
			...
			String newTitle = (String) JOptionPane.showInputDialog(this, "Enter a new title for this window:",
					"QueryForm", JOptionPane.INFORMATION_MESSAGE, null, null, title);
			if (newTitle == null)
				return;
	...
	public void noConnectionAvailableMessage(Integer dataSourceId) {
		if (dataSourceId != null) {
			...
			GUIHelper.errMsg(parent,
					"There are currently no connections available in the connection pool for this data source:\n\n\""
	...
	private void exportResultSet(int exportType, boolean selectedOnly) {
			...
			if (f.exists()) {
				if (f.canWrite() == false) {
					GUIHelper.warningMsg(this,
							"The selected file or destination directory cannot be written. Please choose another output file.",
							"Invalid Selection");
					continue;
				}
				...
				if (option == null || option.equals(AppendOverwriteDialog.CANCEL_OPTION)) {
					setStatusMessage("Export cancelled.");
					return;
				} else if (option.equals(AppendOverwriteDialog.APPEND_OPTION)) {
					append = true;
				}
				break;
			} else {
		...
		if (selectedOnly)
			selectedIndexes = qp.getSelectedIndexes();
		...
		try {
			...
			if (exportType == ExportDialog.EXPORT_INSERT_STATEMENTS) {
				ExportManager.exportInsertStatements(writer, Arrays.asList(currentRowset), selectedIndexes, tableName,
						columnNames, formatters, terminal);
			} else {
				ExportManager.exportDelimited(writer, Arrays.asList(currentRowset), selectedIndexes, ",", columnNames,
						formatters, terminal);
			}
			writer.close();
	...
	public void newTableInfoPanel(TableInfo ti, Integer dataSourceId) throws NoSuchMethodException {
		LocalDataSourceConfig config = (LocalDataSourceConfig) localConfigMap.get(dataSourceId);
		TableInfoPanel tip = new TableInfoPanel(ti, config.getDisplayName());

		String baseName = "(" + dataSourceId + ") " + ti.getTableName() + " [METADATA";

		XInternalFrame jif = new XInternalFrame(baseName + " / " + config.getDisplayName() + "]", true, true, true,
				true);
	...
	MethodComparator tableTypeComparator = new MethodComparator(org.glasser.sql.TableInfo.class, "getTableType", false,
			false, TableInfo.NAME_COMPARATOR, false);

	public void addNewLocalDataSource(LocalDataSourceConfig ld, DataSource ds) throws SQLException {
		...
		try {
			...
			LocalDataSourceConfig config = new LocalDataSourceConfig(ld);
	...
	public void newQueryForm(Integer dataSourceId, TableInfo ti) throws SQLException, NoSuchElementException {
		...
		if (ti.getColumns().length == 0) {
			throw new SQLException(
					"This QueryForm cannot be created because the database is indicating that the table or view has no columns.");
		}
	...
	public void maybeExitProgram() {
		...
		try {
			...
			// System.out.println("Now tempfile is " +
			...
			ex.printStackTrace();
			GUIHelper.errMsg(this,
					"An error occurred while saving the configuration. Changes to the configuration file have not been saved.",
					"Application Error");
			System.exit(-1);
		}
	...
	/**
	 * Invoked when a internal frame has been opened. This is an empty
	...
	/**
	 * Invoked when an internal frame is de-activated. This is an empty
	 * implementation which can be overriden by subclasses.
	 * 
	 * @see javax.swing.JInternalFrame#setSelected
	 */
	...
	/**
	 * Invoked when an internal frame has been closed. This is an empty
	 * implementation which can be overriden by subclasses.
	 * 
	...
	public void internalFrameClosed(InternalFrameEvent e) {
		if (debug)
			System.out.println("TRC: " + getClass().getName() + ".internalFrameClosed()");
	...
	/**
	 * Invoked when an internal frame is de-iconified. This is an empty
	 * implementation which can be overriden by subclasses.
	 * 
	 * @see javax.swing.JInternalFrame#setIcon
	 */
	...
	protected void setColumnsAndKeys(Integer sourceId, TableInfo ti) throws SQLException, NoSuchElementException {
	...
	public void setColumns(Integer sourceId, TableInfo ti) throws SQLException, NoSuchElementException {
		// System.out.println("TRC: " + getClass().getName() + ".setColumns("
		// + sourceId + ", " + ti.getTableName());

		DataSource ds = dsMap.get(sourceId);
		Connection conn = null;
		ResultSet rs = null;
	...
	public void setPrimaryKeys(Integer sourceId, TableInfo ti) throws SQLException {
	...
	public void setForeignKeys(Integer sourceId, TableInfo ti) throws SQLException {
		...
		try {
			...
			rs = dbmd.getImportedKeys(ti.getTableCat(), ti.getTableSchem(), ti.getTableName());
	...
	public void setExportedKeys(Integer sourceId, TableInfo ti) throws SQLException {
		...
		try {
			...
			rs = dbmd.getExportedKeys(ti.getTableCat(), ti.getTableSchem(), ti.getTableName());
	...
	private Set getEditableTypes(String driverClassName) {
		...
		// see if the DriverClassList class knows anything about this driver
		// type.
		...
		// see if any editable types were configured in the file for this driver
		// type
		...
		if (types != null) {
			try {
				...
				while (st.hasMoreTokens()) {
					...
					set.add(i);
					if (debug) {
						System.out.println("Adding editable type " + i + " for driver " + driverClassName);
					}
				}
			} catch (Exception ex) {
