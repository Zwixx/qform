...
public final class QueryPanelEvent implements SmartEvent, java.io.Serializable {
	 ...
	 * 
	 * @param source
	 *            the QueryPanel which originated this event.
	 * @param type
	 *            an it indicating what type of event occurred. Currently the
	 *            only type of event is STATE_CHANGED.
	 */
	...
	/**
	 * Returns the Class for the interface that receives this type of event; in
	 * this particular case is is QueryPanelListener.class.
	 */
	public Class<QueryPanelListener> getListenerClass() {
	...
	/**
	 * This method, specified by the org.glasser.util.SmartEventListener
	 * interface, is passed a reference to one of the listeners registered to
	 * receive this event. It then calls the appropriate method on the listener
	 * interface, notifying the object of the event.
	 */
	public void notifyListener(EventListener listener) {

		QueryPanelListener panelListener = (QueryPanelListener) listener;

		switch (type) { // a switch is used in anticipation of adding more event
						// types.
		case STATE_CHANGED:
