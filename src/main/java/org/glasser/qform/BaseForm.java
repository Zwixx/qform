...
/**
 * An object of this class provides the form through which query parameters are
 * input and query results are displayed (one record at a time) in the QueryForm
 * app.
 */
public class BaseForm extends JPanel {
	...
	public BaseForm(Vector<TextBox> fieldList) {
		...
		for (int row = 0; row < fields.length; row++) {
			...
			// we need to see how many lines high the JTextArea needs to be to
			// display
			// the field's contents, assuming that it is 40 characters wide.
			// This only
			...
			// if the number of lines required to display this field is more
			// than
			// 7, we'll only make it 7 high, but put it in a scrollpane so all
			// of
	...
	public void setFieldsEditable(boolean b) {
		for (int j = 0; j < fields.length; j++) {
			...
			if (b)
				GUIHelper.setTabTraversal(fields[j]);
		}
	}
	...
	public boolean isAtBeginning() {
		if (resultSetBuffer != null)
			return resultSetBuffer.isAtBeginning();
		return false;
	}
	...
	public String getPkWhereClause(boolean includeWhere) throws UnknownPrimaryKeyException {
		if (resultSetBuffer == null)
			throw new RuntimeException("There is no current record.");

		// collect the indices of all of the PK fields.
		ArrayList<Integer> pkFieldIndices = new ArrayList<>();
		for (int j = 0; j < fields.length; j++) {
			if (fields[j].isPkComponent()) {
				pkFieldIndices.add(j);
			} else if (hasPK == false) {
				...
				if (fields[j].isTypeDisplayable()
						...
						&& ((DBUtil.isCharType(dataType) && fields[j].getLength() < 51) || DBUtil
		...
		Vector<String> row = resultSetBuffer.getCurrentRow();
	...
	public int next() throws SQLException {
		...
		if (cachedRow == null) {
			...
			// if we're not at the end of the backbuffer, read in the next row
			// from it.
			for (int j = 0; j < fields.length; j++) {
				Object fieldValue = cachedRow.elementAt(j);
				fields[j].setValue(fieldValue);
	...
	public boolean first() {
		...
		if (resultSetBuffer.isAtBeginning())
			return false;
