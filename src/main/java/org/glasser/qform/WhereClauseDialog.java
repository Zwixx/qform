...
public class WhereClauseDialog extends JDialog implements ActionListener {
	 ...
	 * results of the editing, and can be retrieved by the getWhereClause()
	 * method. It will always be non-null if the user clicks Submit; it will be
	 * an empty String ("") if the user clicks Submit without entering anything.
	 * It will be null if the user closes the dialog or clicks Cancel.
	 */
	private String whereClause = null;

	private java.util.List<String> historyList = null;
	...
	public void setVisible(boolean b) {
		if (b)
			throw new RuntimeException("WhereClauseDialog must be opened with the openDialog() method.");
	}
	...
	/**
	 * This opens the dialog in a modal state, with the last item in the
	 * historyList displayed for editing. When this method returns, the results
	 * of the user editing can be retrieved with getWhereClause().
	 */
	public void openDialog(String tableName, java.util.List<String> historyList) {
		lblHeader.setText("SELECT * FROM " + tableName + " WHERE ");
		this.tableName = tableName;
		this.historyList = historyList;
		pointer = historyList.size() - 1;
		if (pointer > -1)
			txtInput.setText(historyList.get(pointer));
		else
			txtInput.setText(null);
		setModal(true);
		txtInput.requestFocus();
	 ...
	 * The returned String will always be non-null if the user clicks Submit; it
	 * will be an empty String ("") if the user clicks Submit without entering
	 * anything. It will be null if the user closes the dialog or clicks Cancel.
	 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals("SUBMIT")) {
			...
			if (historyList.size() == 0 || pointer == 0) {
